...
{
	FConsolePanel* console = (FConsolePanel*)data->UserData;
	return console->TextEditCallback(data);
}

static int   Strnicmp(const char* s1, const char* s2, int n) { int d = 0; while (n > 0 && (d = toupper(*s2) - toupper(*s1)) == 0 && *s1) { s1++; s2++; n--; } return d; }

bool FConsolePanel::Init()
{
...
void FConsolePanel::OnRender()
{
	...
	if (ImGui::BeginChild("ScrollingRegion", ImVec2(0, -footer_height_to_reserve), ImGuiChildFlags_NavFlattened, ImGuiWindowFlags_HorizontalScrollbar)) // ImGui Window �ȿ� Child Window�� ����
	{
...
void FConsolePanel::ExecCommand(const FString& CommandLine)
{
	...
	{
		FString Rest = CommandLine.substr(10); // 앞 10글자 빼고 parsing
		...
		else
		{
			...
			try
			{
				// UUID / TEXT / Timer Setting, 실제 Actor 조회는 Render loop에서 실행
...
int FConsolePanel::TextEditCallback(ImGuiInputTextCallbackData* data)
{
